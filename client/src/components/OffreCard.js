// import React from "react";
// import { Card, Image, Button, Col } from "react-bootstrap";
// import { useDispatch, useSelector } from "react-redux";
// import {
//   deleteOffre,
//   getOffre,
//   toggleTrue,
// } from "../JS/actions/indexOffre";
// import { Link } from "react-router-dom";
// import './OffreCard.css'
// const OffreCard = ({ offre }) => {
//   const dispatch = useDispatch();
//   const isAuth = useSelector((state) => state.userReducer.isAuth);
//   return (
//     // <div
//     //   style={{
//     //     textAlign: "center",
//     //     display: "flex",
//     //     justifyContent: "center",
//     //     flexWrap: "warp",
//     //   }}
//     // >
//     //   <Card
//     //     style={{
//     //       width: "18rem",
//     //       height: "25rem",
//     //       marginRight: "30px",
//     //       marginBottom: "20px",
//     //       marginTop: "30px",
//     //       backgroundColor: "white",
//     //       borderRadius: "8px",
//     //       border: "transparent",
//     //       boxShadow: "0 10px 10px 0 rgba(0,0,0,0.2)",
//     //     }}
//     //   >
//     //     <Card.Header
//     //       style={{
//     //         borderTopRightRadius: "8px",
//     //         borderTopLeftRadius: "8px",
//     //         backgroundColor: "#277fa5",
//     //         height: "13rem",
//     //       }}
//     //     />
//     //     <Col>
//     //       <Image
//     //         src="data:image/png;base64,"
//     //         roundedCircle
//     //         style={{
//     //           height: "150px",
//     //           width: "150px",
//     //           position: "relative",
//     //           bottom: "80px",
//     //           border: "10px solid white",
//     //           backgroundColor: "transparent",
//     //         }}
//     //       /> 
//     //     </Col>
//     //     <Card.Body style={{ position: "relative", bottom: "90px" }}>
//     //       <Card.Title style={{ margin: "0", color: "#505151" }}>
//     //         {offre.title}
//     //       </Card.Title>
//     //       <Card.Text style={{ fontSize: "small", color: "#4baed4" }}>
//     //         {offre.description}
//     //       </Card.Text>
//     //       <Card.Text style={{ fontSize: "small", color: "#4baed4" }}>
//     //         {offre.dateDep}
//     //       </Card.Text>
//     //       <Card.Text style={{ fontSize: "small", color: "#4baed4" }}>
//     //         {offre.dateFin}
//     //       </Card.Text>
//     //       <Card.Text style={{ fontSize: "small", color: "#4baed4" }}>
//     //         {offre.salaire}
//     //       </Card.Text>
//     //       <Card.Text style={{ fontSize: "small", color: "#4baed4" }}>
//     //         {offre.emplacement}
//     //       </Card.Text>
//     //       <Card.Text style={{ fontSize: "small", color: "#4baed4" }}>
//     //         {offre. experience}
//     //       </Card.Text>
//     //       <Card.Text style={{ fontSize: "small", color: "#4baed4" }}>
//     //         {offre.niveauEtude}
//     //       </Card.Text>
//     //     </Card.Body>
//     //     <div className="buttons">
//     //       <Link
//     //         to="/Edit_Offre"
//     //         onClick={() => {
//     //           dispatch(getOffre(offre._id));
//     //           dispatch(toggleTrue());
//     //         }}
//     //       >
//     //         <Button variant="outline-primary edit-button">Edit</Button>
//     //       </Link>

//     //       <Button
//     //         variant="outline-danger edit-button"
//     //         onClick={() => dispatch(deleteOffre(offre._id))}
//     //       >
//     //         Delete
//     //       </Button>
//     //     </div>
//     //   </Card>
//     // </div>

// <div className="container">
// <React.Fragment>
//     <div className="row no-gutters shadow-lg p-2 mb-3 bg-white rounded"  >  
//         <div className="col-md-4">

//           {/* <img  src={offre.logo} className="card-img App-img" alt={offre.titre} width="120px" height="160px" /> */}
//         </div>
// {/*         
//           <div className="col-md-7">
//           <div className="card-body">
//             <h5 className="card-title btn-link text-dark" >{offre.titre}</h5>
//             <p className="card-text"><i className = "fa fa-building-o "> {offre.description} </i></p>
//             <p className="card-text"><i class="fa fa-calendar" aria-hidden="true"> {offre. experience} </i></p>
//             <p className="card-text"><i className="fa fa-money"> {offre.salaire}</i></p>
//             <p className="card-text"><i className="fa fa-map-marker"> {offre.emplacement} </i></p>
//             <p className="card-text"><small className="text-muted">{offre.dateDep}{offre.dateFin}</small></p>
//             <p className="card-text"><small className="text-muted">{offre.niveauEtude}</small></p>   
//             { isAuth? 
//             (<button type="button" class="btn btn-danger">Postuler</button> ):
//             (<Link to="/login"><button type="button" class="btn btn-danger"> Postuler</button> </Link>)

//             }
//                */}
//           </div>
//         </div>
//     </div>
//  </React.Fragment>
// </div>




//   );
// };

// export default OffreCard;
